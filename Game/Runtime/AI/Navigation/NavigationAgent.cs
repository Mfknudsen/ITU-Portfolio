...
namespace Runtime.AI.Navigation
{
    ...
    public sealed class NavigationAgent : MonoBehaviour
    {
        ...
        [ShowInInspector] [ReadOnly] private int agentID = -1, groupID = -1;
        ...
        private IEnumerator Start()
        {
            ...
            while (!UnitNavigation.Ready)
            {
                Debug.Log(UnitNavigation.Ready);
                yield return 1;
            }
        ...
        internal void UpdatePositionFromEcs(EntityManager entityManager)
        {
            if (!this.entity.HasValue) return;

            UnitAgentComponent agentComponent = entityManager.GetComponentData<UnitAgentComponent>(this.entity.Value);

            this.transform.position = agentComponent.Position;
            this.transform.rotation = agentComponent.Rotation;
        }

        internal void UpdatePositionInEcs(EntityManager entityManager)
        {
            if (!this.entity.HasValue) return;

            UnitAgentComponent agentComponent = entityManager.GetComponentData<UnitAgentComponent>(this.entity.Value);
            agentComponent.Position = this.transform.position;
            agentComponent.Rotation = this.transform.rotation;
            entityManager.SetComponentData(this.entity.Value, agentComponent);
        }
